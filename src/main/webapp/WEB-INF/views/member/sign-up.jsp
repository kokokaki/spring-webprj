...
<script>
    ...
    $(document).ready(function () {
        ...
        $idInput.on('keyup', e => {
            ...
            //아이디를 패턴에 맞게 입력하지 않았을 경우
            //test메서드는 정규표현식을 검증하여 값이 정규표현식과 일치하면 true
            //일치하지 않으면 false를 리턴
            else if (!getIdCheck.test($idInput.val())) {
                $idInput.css('background', 'pink');
                $('#idChk').html('<b style="font-size:14px; color:red;">[영문, 숫자 4~14자로 작성하세요.]</b>');
                chk1 = false;
            }
            //아이디 중복확인 체크
            else {
                //비동기 통신
                fetch('/check?kind=account&info=' + $idInput.val())
                    .then(res => res.text())
                    .then(flag => {
                        //console.log(flag);
                        if (flag === 'true') {
                            ...
                            $('#idChk').html(
                                '<b style="font-size:14px; color:green;">[사용가능한 아이디입니다.]</b>');
                            chk1 = true;
                        }
                    });
            }
        ...
        $emailInput.on('keyup', function () {
            ...
            //이메일값 유효성검사
                ...
                //이메일 중복확인 비동기 통신
                fetch('/check?kind=email&info=' + $emailInput.val())
                    .then(res => res.text())
                    .then(flag => {
                        //console.log(flag);
                        if (flag === 'true') {
